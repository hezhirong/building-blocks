...
module.exports = async function(user, projectId) {
    try {
        ...
        let projectDir = path.join(
                __dirname,
                `./exportDirectory/${user.name}/${data.projectName}`
            ),
        ...
        var html = shell.cat(
            path.join(__dirname, "./static/pages/preview.html")
        );
        ...
        // 修改webpackconfig
        config.entry.app = path.join(
            __dirname,
            `./exportDirectory/${user.name}/${data.projectName}/index.js`
        );
        config.output.path = path.join(
            __dirname,
            `./exportDirectory/${user.name}/${data.projectName}/dist`
        );
        // 去除preview的配置
